...
from .network import sanitize_proxy_environment
...
async def create_chat_completion(
    ...
    response = ""
    # create response
    attempts = max(1, int(max_attempts)) if max_attempts is not None else (1 if (stream and websocket is not None) else 10)
    last_exception: Exception | None = None
    for attempt in range(1, attempts + 1):
        try:
            call = provider.get_chat_response(messages, stream, websocket, **kwargs)
            response = await asyncio.wait_for(call, timeout=request_timeout_seconds) if request_timeout_seconds else await call
        except Exception as exc:
            last_exception = exc
            logging.getLogger(__name__).warning(
                f"LLM request failed (attempt {attempt}/{attempts}): {exc}"
            )
            if attempt < attempts:
        ...
        if not response:
            ...
            logging.getLogger(__name__).warning(
                f"LLM returned empty response (attempt {attempt}/{attempts})"
            )
            if attempt < attempts:
                await asyncio.sleep(min(2 ** (attempt - 1), 8))
                continue
