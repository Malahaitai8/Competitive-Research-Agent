...
- Suitable for both digital viewing and printing"""
    ...
    def generate_search_queries_prompt(
        ...
        competitive_mode_prompt = ""
        if "[COMPETITIVE_RESEARCH_MODE]" in task:
            competitive_mode_prompt = """
This is a competitive product research task. Generate search queries that:
- First validate the competitor entities before comparing features: parent company, brand history, acquisition, merger, rename, rebrand, business line ownership, and whether old/new brands should be compared separately.
- Include at least one query focused on official/company sources for the entity relationship and brand scope. Use natural language phrases such as "official", "company profile", "businesses", "brand upgrade", "parent company", "acquisition", "rename", and the competitor names.
- Cover every competitor explicitly named in the task.
- Cover the selected research dimensions with a consistent comparison lens.
- Include the region and time range when they are relevant.
- Prefer public, verifiable facts from official websites, pricing pages, help centers, changelogs, release notes, official announcements, app store pages, and credible technology media.
- For pricing, membership, brand ownership, market share, and recent updates, generate queries that can discover official sources first. Use third-party media only as supplemental evidence.
- Include queries for pricing, membership benefits, recent product updates, product positioning, target users, business model, and core features when those dimensions are requested.
- Avoid duplicate queries and avoid broad generic questions that cannot be verified from public sources.
- Do not use the marker [COMPETITIVE_RESEARCH_MODE] in any generated query.
"""
...
"""
        ...
        if "[COMPETITIVE_RESEARCH_MODE]" in question:
            competitive_report_prompt = """
This is a competitive product research report. Ignore the marker [COMPETITIVE_RESEARCH_MODE] in the task text.

You MUST use this report structure:
# {动态报告标题}
## 1. 研究范围与口径
## 2. 竞品主体关系与品牌口径
## 3. 竞品概览
...
- The title must be based on the user's research topic. Do not hard-code "AI 产品" unless the topic is actually about AI products. If the topic already contains "竞品研究", use it directly as the title or append "报告" only once; otherwise use "{研究主题}竞品研究报告".
- In section 2, explicitly validate each competitor entity: parent company, current public-facing brand, previous brand names, acquisition/merger/rebrand relationship, and whether two names should be treated as one entity or separate competitors.
- If a competitor has been acquired, renamed, rebranded, merged, or folded into another product, clearly state the relationship before comparing features.
- Compare all competitors using the same dimensions wherever possible.
- 用户输入的所有竞品必须出现在核心功能对比矩阵中。不要因为缺少官方来源、公开信息不足或名称需要确认而省略某个竞品。
- The core comparison matrix must use the user-provided competitor names as columns or rows, including names that have limited evidence. For a competitor or cell with no verified public information, write "暂未找到官方公开信息" or "公开信息不足，需确认产品名称或来源" in the cell instead of dropping it from the matrix.
- 矩阵单元格默认使用官方来源支撑的信息。If a cell relies on non-official or third-party information, write the finding cautiously in the cell, but do not create a separate source/status column.
- 不要为矩阵单独增加“来源”列、"source" column, "official source" column, or "confidence" column. Instead, 在矩阵下方统一标注 which competitors or dimensions relied on non-official sources or had insufficient public information.
- Separate facts, analysis, and recommendations.
- Pricing, membership benefits, feature availability, and recent updates must include source citations and collection or publication dates when available.
- Prefer official sources for key facts. Parent-company relationships, brand history, pricing, membership, and business model claims should use official sources whenever available.
- If no official source is found for a key fact, explicitly write "未找到官方来源，以下为第三方来源信息" before using third-party evidence.
- Respect the user's time range strictly. Put facts outside the requested time range in "背景信息" or "研究限制", not in "近期产品更新".
- If reliable public information is not found, write “暂未找到公开信息” instead of guessing.
- If sources conflict, explicitly call out the conflict.
- Product opportunity points must explain the evidence and reasoning behind them.
- Write the report in Chinese unless the user explicitly requested another language.
"""
