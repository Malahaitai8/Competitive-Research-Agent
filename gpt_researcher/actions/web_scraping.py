...
from ..competitive_sources import filter_usable_source_urls, is_usable_source_url
...
async def scrape_urls(
    ...
    urls = filter_usable_source_urls([str(url) for url in urls or []])
    if not urls:
        return [], []
...
async def filter_urls(urls: list[str], config: Config) -> list[str]:
    ...
    for url in urls:
        ...
        if is_usable_source_url(url) and not any(excluded in url for excluded in config.excluded_domains):
