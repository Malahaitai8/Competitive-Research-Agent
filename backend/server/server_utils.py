...
from .competitive_research import collect_competitive_metadata, save_competitive_analysis
from .progress_events import make_progress_event, normalize_progress_event
...
class CustomLogsHandler:
    ...
    async def send_json(self, data: Dict[str, Any]) -> None:
        ...
        data = normalize_progress_event(data)
...
async def handle_start_command(websocket, data: str, manager):
    ...
    stream_result = await manager.start_streaming(
        task,
        report_type,
        report_source,
        source_urls,
    ...
    if isinstance(stream_result, tuple):
        report, researcher_obj = stream_result
    else:
        report = stream_result
    report = str(report)
    file_paths = await generate_report_files(report, sanitized_filename)
    agent_metadata = collect_competitive_metadata(researcher_obj)
    competitive_analysis_path, competitive_analysis = await save_competitive_analysis(
        task,
        report,
        sanitized_filename,
        logs_handler.log_file,
        agent_metadata,
    )
    if competitive_analysis_path:
        file_paths["competitive_analysis"] = competitive_analysis_path
        file_paths["competitive_analysis_data"] = competitive_analysis
...
async def generate_report_files(report: str, filename: str) -> Dict[str, str]:
    async def generate_one(label: str, writer) -> str:
        try:
            path = await writer(report, filename)
            if not path:
                logger.warning("Report %s export did not produce a file.", label)
            return path or ""
        except Exception as exc:
            logger.warning("Report %s export failed: %s", label, exc, exc_info=True)
            return ""

    md_path = await generate_one("markdown", write_text_to_md)
    docx_path = await generate_one("docx", write_md_to_word)
    pdf_path = await generate_one("pdf", write_md_to_pdf)
            ...
            try:
                ...
                if data == "ping":
                    ...
                    await websocket.send_json(
                        make_progress_event(
                            "warning",
                            "任务正在运行中，请稍后再试",
                            raw_message="Task already running. Please wait.",
                            stage="task_control",
                            status="blocked",
                            severity="warning",
                        )
                ...
                # Normalize command detection by checking startswith after stripping whitespace
                    ...
                    await websocket.send_json(make_progress_event(
                        "error",
                        "服务端无法识别该指令，请重新发起任务",
                        raw_message="Unknown command received by server",
                        stage="task_control",
                        status="failed",
                        severity="error",
                    ))
