...
from backend.server.agent_repair import run_competitive_agent_repair
from backend.server.competitor_normalization import normalize_competitive_task_with_llm
from backend.server.semantic_remediation import run_competitive_semantic_remediation
...
class BasicReport:
    ...
    async def run(self):
        self.query, _ = await normalize_competitive_task_with_llm(self.gpt_researcher, self.query)
        await self.gpt_researcher.conduct_research()
        await run_competitive_agent_repair(self.gpt_researcher, self.query)
        report = await self.gpt_researcher.write_report()
        report, _ = await run_competitive_semantic_remediation(self.gpt_researcher, self.query, report)
        return report
